...
class FileManager:
    ...
    def find_safetensor_files(self) -> List[Path]:
        ...
        logger.info(
            f"Found {len(unique_files)} safetensor file(s) in {self.folder_path}"
        )
        return unique_files
    ...
    def analyze_directory(self) -> Tuple[List[Path], List[Path]]:
        ...
        for st in safetensors:
            ...
            if data and self.get_sha256_from_json(data):
                ...
                need.append(st)
        logger.info(
            f"Directory analysis: {len(need)} need hashing, {len(have)} have hashes"
        )
        return need, have
    ...
    def cleanup_orphaned_files(self) -> Dict[str, int]:
        ...
        existing_safetensors = {p.resolve() for p in self.find_safetensor_files()}
        ...
        for f in jsons:
            ...
            if st not in existing_safetensors:
        ...
        logger.info(
            f"Cleaned {cleaned['json']} JSON and {cleaned['preview']} preview files"
        )
        return {
            "json_files_cleaned": cleaned["json"],
            "preview_files_cleaned": cleaned["preview"],
        }
